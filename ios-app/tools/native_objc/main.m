...
- (void)mountWebView {
    @try {
        ...
        NSString *flags = [NSString stringWithFormat:
            ...
            "window.GONGTIAN_WEB_DEPLOYMENT=false;"
            "var href=String(location.href||'');"
            "var isBaowen=/\\/baowen\\//i.test(href)||!!document.querySelector('.shell,.notice-board,.auth-wrap');"
            ...
            "if(%@!==''){window.GONGTIAN_REMOTE_API_BASE=%@;window.__GONGTIAN_API_BASE__=%@;}"
            "window.__INSULATION_DEVICE_ID__=%@;}catch(e){}})();",
            JSONString(deviceId),
            JSONString(insApi), JSONString(insApi), JSONString(insApi),
            JSONString(gtApi), JSONString(gtApi), JSONString(gtApi),
            JSONString(deviceId)];
        ...
        NSString *adaptiveJs =
            ...
            "var isBaowenPath=/\\/baowen\\//i.test(href)||!!document.querySelector('.shell,.notice-board,.auth-wrap');"
            ...
            /* 工天外壳：保持安卓同款 100% 高 iframe，勿套保温 scroll */
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    NSString *js = [NSString stringWithFormat:
        ...
        "document.documentElement.style.setProperty('--sab','%dpx');"
        "var href=String(location.href||'');"
        "if(/\\/baowen\\//i.test(href)||document.querySelector('.shell,.notice-board,.auth-wrap'))"
